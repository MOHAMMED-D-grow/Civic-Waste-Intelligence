...
4. STRICT PROHIBITION: Do NOT output any severity rating, risk score, priority level, impact score, confidence percentage, danger level, or arbitrary numerical scoring. Keep the response factual, concise, and focused on the visible evidence.`;
    ...
    // Resilient fallback candidate models in case of temporary 503 high demand spikes
    const candidateModels = ["gemini-3.8-flash", "gemini-3.1-flash-lite", "gemini-flash-latest"];
    let responseText = "";
    let lastError: unknown = null;

    for (const modelName of candidateModels) {
      // Try up to 2 attempts per model with backoff if experiencing high demand
    ...
    const explanation = typeof parsed.explanation === "string" && parsed.explanation.trim().length > 0
      ...
      : isDetected
        ? "Visible open-area waste and refuse accumulation was detected in the submitted evidence."
        : "No significant accumulation of open-area waste or litter was detected in the image.";
    ...
    // Clean up raw JSON error messages from SDK
...
// In-memory cache for geocoding queries with TTL to keep responses fast and comply politely with Nominatim policy
...
function getFromGeocodeCache(key: string) {
...
function setInGeocodeCache(key: string, data: any) {
...
// Fallback suggestions for common Coimbatore municipal hubs in case of offline/network issues
const COIMBATORE_FALLBACK_SUGGESTIONS = [
  {
    place_id: "cb-1",
    lat: "11.0169",
    lon: "76.9602",
...
// Enhanced Address Auto-Complete Suggestion Service via Nominatim
app.get("/api/geocode/autocomplete", async (req, res) => {
  try {
    ...
    // Check if query should prioritize Coimbatore or append city if not explicitly provided
    ...
    // Bias to coordinates if provided
    ...
    // If no results with appended city, try the original query without appending city
    ...
    // Format results into clean, street-level AddressSuggestion objects
    let suggestions = (items || []).map((item: any) => {
      ...
      // Formulate secondary street/locality details
      const subtitleParts = [
    ...
    // If Nominatim returned no items, check if any local fallback keywords match
...
// Proxy for Nominatim geocoding to avoid CORS and handle user rate-limiting politely
